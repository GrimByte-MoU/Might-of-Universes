...
namespace MightofUniverses.Content.Items.Buffs
{
    public class HellsMark : ModBuff
    {
        ...
        {
            // Deal 80 damage per second
            if (Main.GameUpdateCount % 15 == 0)
            {
                npc.StrikeNPC(new NPC.HitInfo { Damage = 20, Knockback = 0 });
            }
            
            // Visual effect - hellish mark
            if (Main.rand.NextBool(2))
            {
                ...
                // Occasionally add a pentagram-like effect
        ...
        {
            // Deal 50 damage per second
            player.lifeRegen -= 100; // -100 life regen = 50 damage per second
    ...
    // This class handles the explosion effect when magic hits a marked enemy
    public class HellsMarkGlobalProjectile : GlobalProjectile
    {
        public override void OnHitNPC(Projectile projectile, NPC target, NPC.HitInfo hit, int damageDone)
        {
            // Check if the projectile is magic and the target has Hell's Mark
            if (projectile.DamageType == DamageClass.Magic && target.HasBuff(ModContent.BuffType<HellsMark>()))
            {
                ...
                // Deal additional damage (2x the original damage)
